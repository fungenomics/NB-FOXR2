...
further filtering on genes or peaks is performed. We also load in the TF analysis (which estimates a total signal for each TF in each cell) from Cellranger, in case we want to use it downstream.
...
# subset to peaks not on the custom chromosomes, since those will interfere with
# peak counting; and we're not interested in chromatin accessibility on those
# chromosomes
counts_atac <- counts$Peaks[!grepl("p53_Cas9|Akaluc|piggybac|P2A|ATRX", rownames(counts$Peaks)),]
seurat_prefilt[["ATAC"]] <- CreateChromatinAssay(
    counts = counts_atac, 
    sep = c(":", "-"),
    genome = seqinfo_mm10, # obtained from config$genome_info
    fragments = file.path(config$cellranger_dir, "atac_fragments.tsv.gz"),
    min.cells = config$min_cells
)

# set gene annotations
Annotation(seurat_prefilt[["ATAC"]]) <- annotation_mm10 # obtained from config$genome_info
...
# set gene annotations
Annotation(seurat[["peaks"]]) <- annotation_mm10 # obtained from config$genome_info
